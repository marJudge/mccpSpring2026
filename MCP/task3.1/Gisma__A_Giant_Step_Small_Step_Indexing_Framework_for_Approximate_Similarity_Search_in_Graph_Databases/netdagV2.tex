...
Based on Lemma~\ref{}, we propose our NetDag index, which uses two different approaches to index $\mathcal U$ when $d>\alpha$ and $d<\alpha$.
...
\begin{itemize}
\item if $i\geq \alpha$, $H_i.r = 2^{i-\alpha}\alpha$ and 
\begin{itemize}
\item the set of nodes at the $i$-th layer is a $H_i.r$-net of $D$;
\item a node $u$ at the $(i+1)$-th layer has an edge to each node $v$ at the $i$-th layer if $d(u,v)\leq 1.5H_i.r+2\tau$;
\end{itemize}
\item else, $H_i.r = i$ and 
\begin{itemize}
\item a node $v$ at the $i$-th layer corresponds to a ball $B(v, H_i.r)$;
\item a node $u$ at the $(i+1)$-th layer has a child $v$ for each point $v$ located at the shell of $B(u, 2\tau+1)$;
\item the nodes at the $i$-th layer are in $X$ but may not in $D$.
\end{itemize}
\end{itemize}
\end{definition}
